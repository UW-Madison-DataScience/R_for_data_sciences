...
This is an example using the `dplyr` package to access SQLite. It draws on lessons from the [UW-Madison Data Carpentry Workshop](http://uw-madison-aci.github.io/2016-06-01-uwmadison/). Other resources include the [Data Wrangling Cheat Sheet](https://www.rstudio.com/resources/cheatsheets/) and the [Databases using dplyr](https://db.rstudio.com/dplyr/). Note that the 0.5.0 release of `dplyr`, and introduction of `dbplyr` package, dramatically changed the connections to databases. See [dplyr compatibility](http://dplyr.tidyverse.org/articles/compatibility.html) for notes on this.
...
Make sure you have the package `dplyr`. You also need the back `dbplyr` to use SQL features.

```
install.packages("dplyr","dbplyr")
```
...
```{r}
tbl(sql_connect, dbplyr::build_sql(
